...
function AcceptInviteContent() {
  ...
  useEffect(() => {
    if (!triedAutoJoin && token) {
      ...
      accept(token)
        .then((result) => {
          const inviteData = result.data?.acceptInvite;
          ...
          // User exists and joined successfully, redirect
          if (inviteData.appId) {
            // For app invites, redirect to the specific app
            router.replace(`/dashboard/${inviteData.organizationId}/apps/${inviteData.appId}`);
          } else if (inviteData.organizationId) {
  ...
  const handleSetupJoin = async () => {
    try {
      ...
      const result = await accept(token, username.trim(), password);

      if (result.data?.acceptInvite?.appId) {
        // For app invites, redirect to the specific app
        router.replace(`/dashboard/${result.data.acceptInvite.organizationId}/apps/${result.data.acceptInvite.appId}`);
      } else if (result.data?.acceptInvite?.organizationId) {
        // For organization invites, redirect to the organization dashboard
        router.replace(`/dashboard/${result.data.acceptInvite.organizationId}`);
      } else {
        throw new Error("Invalid response from server");
      }
    } catch (err: unknown) {
