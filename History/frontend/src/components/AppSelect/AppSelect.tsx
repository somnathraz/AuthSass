...
export default function AppSelect() {
  ...
  // Set default selection to first app if available and no app is selected
  useEffect(() => {
    if (!loading && data?.myApps && data.myApps.length > 0 && !selectedAppId) {
      setSelectedAppId(data.myApps[0].id);
    }
  }, [loading, data, selectedAppId, setSelectedAppId]);
          ...
          <form onSubmit={handleFormSubmit} className="mt-4 space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded-md p-2 text-sm bg-white text-black focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
            <div>
