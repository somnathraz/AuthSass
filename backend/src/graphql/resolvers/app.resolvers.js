...
const { auditLog, auditCustomerLog } = require("../../utils/audit");
const {
  toGraphQLId,
  sanitizeObjectIds,
  sanitizeArrayIds,
} = require("../../utils/idHelpers");
const {
...
const appResolvers = {
  Query: {
      ...
      try {
        ...
        if (filter) {
          ...
          if (filter.organizationId) {
            ...
            console.log(
              `🎯 Filtering apps for specific organization: ${organizationIdFilter}`
            );
          }
        }
        ...
        if (user.role !== "ADMIN" && user.role !== "SUPER_ADMIN") {
          ...
          if (!organizationIdFilter) {
            console.log(
              "📁 No specific org filter - getting all user organizations"
            );
            ...
            if (orgIds.length > 0) {
              ...
              console.log(
                `📂 User has access to ${orgIds.length} organizations:`,
                orgIds
              );
            } else {
            ...
            console.log(
              `🔐 Verifying user access to organization: ${organizationIdFilter}`
            );
            const userOrgs = await OrganizationService.getUserOrganizations(
              user.id
            );
            const hasAccess = userOrgs.some(
              (org) => org.id === organizationIdFilter
            );

            if (!hasAccess) {
              console.log(
                `❌ User does not have access to organization: ${organizationIdFilter}`
              );
              throw new ForbiddenError("Access denied to this organization");
            }
            console.log(
              `✅ User has access to organization: ${organizationIdFilter}`
            );
          }
        } else {
          ...
          if (organizationIdFilter) {
            console.log(
              `👑 Admin filtering for specific organization: ${organizationIdFilter}`
            );
          } else {
        ...
        console.log(
          `✅ Found ${rawApps.length} apps from database (total: ${total})`
        );
        if (rawApps.length > 0) {
          console.log("📋 Apps found:");
          rawApps.forEach((app, index) => {
            console.log(
              `  ${index + 1}. "${app.name}" in org ${app.organizationId}`
            );
          });
        }
          ...
          .map((app) => {
            try {
              ...
              if (!sanitizedApp.organizationId) {
                ...
                sanitizedApp.organizationId =
                  sanitizedApp.organizationId.toString();
              }
              ...
              if (
                ...
                console.error(
                  "App still missing required fields after sanitization:",
                  {
                    id: sanitizedApp.id,
                    name: sanitizedApp.name,
                    organizationId: sanitizedApp.organizationId,
    ...
    async appMembers(parent, { appId }, context) {
      ...
      const members = memberships
        .filter(
          (membership) =>
            membership.user._id.toString() !== app.owner._id.toString()
        )
        .map((membership) => ({
      ...
      try {
        ...
        if (!membership && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to view API keys for this application"
          );
        }
    ...
    async appLogs(
      parent,
      { appId, limit = 50, offset = 0, eventType, dateFrom, dateTo },
      context
    ) {
      const { user } = context;
  ...
  Mutation: {
    async createApp(parent, { input }, context) {
      ...
      console.log("createApp called with:", {
        input,
      ...
      try {
        ...
        if (
          ...
          throw new ForbiddenError(
            "Insufficient permissions to create app in this organization. Only organization administrators can create applications."
          );
        }
        ...
        // Audit log: Customer tier for app creation
        await auditCustomerLog("APP_CREATED", organizationId, user.id, {
          category: "APPLICATION",
          actorType: "USER",
          description: `Application "${app.name}" created`,
        ...
        const populatedApp = await App.findById(app._id).lean(); // Don't populate owner here, let field resolver handle it
        ...
        if (!appId) {
          ...
          throw new Error(
            "Failed to create application - invalid ID generated"
          );
        }

        const sanitizedApp = sanitizeObjectIds(populatedApp, [
          "organizationId",
        ]); // Don't sanitize id here
    ...
    async updateApp(parent, { id, input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to update this application"
          );
        }
        ...
        // Audit log: Customer tier for app update
        await auditCustomerLog("APP_UPDATED", app.organizationId, user.id, {
          category: "APPLICATION",
          actorType: "USER",
          description: `Application "${app.name}" updated`,
          applicationName: app.name,
          applicationId: id,
    ...
    async deleteApp(parent, { id }, context) {
      ...
      try {
        ...
        if (!isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Only application owner or system admin can delete application"
          );
        }
        ...
        // Audit log: Customer tier for app deletion
        await auditCustomerLog("APP_DELETED", app.organizationId, user.id, {
          category: "APPLICATION",
          actorType: "USER",
          description: `Application "${app.name}" deleted`,
          applicationName: app.name,
          applicationId: id,
          severity: "HIGH",
        });
    ...
    async addAppMember(parent, { input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to add members to this application"
          );
        }
    ...
    async removeAppMember(parent, { input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to remove members from this application"
          );
        }
    ...
    async updateAppMemberRole(parent, { input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to update member roles in this application"
          );
        }
    ...
    async createApiKey(parent, { input }, context) {
      ...
      try {
        ...
        if (!membership && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to create API keys for this application"
          );
        }
    ...
    async archiveApp(parent, { appId }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to archive this application"
          );
        }
    ...
    async unarchiveApp(parent, { appId }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to unarchive this application"
          );
        }
    ...
    async bulkUpdateAppStatus(parent, { appIds, status }, context) {
      ...
      try {
        ...
        if (user.role !== "ADMIN" && user.role !== "SUPER_ADMIN") {
          throw new ForbiddenError(
            "Only system administrators can perform bulk operations"
          );
        }
    ...
    async transferAppOwnership(parent, { appId, newOwnerId }, context) {
      ...
      try {
        ...
        if (!isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Only application owner or system admin can transfer ownership"
          );
        }
    ...
    async cloneApp(parent, { appId, name, organizationId }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to clone this application"
          );
        }
        ...
        const hasTargetOrgAccess =
          await OrganizationService.checkUserPermission(user.id, targetOrgId, [
            "ADMIN",
          ]);
        ...
        if (
          ...
          throw new ForbiddenError(
            "Insufficient permissions to create app in target organization"
          );
        }
    ...
    async regenerateApiKey(parent, { input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to regenerate API keys"
          );
        }
    ...
    async updateAppGeneralSettings(parent, { id, input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to update app settings"
          );
        }
    ...
    async updateAppAuthSettings(parent, { id, input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to update app auth settings"
          );
        }
    ...
    async updateAppSecuritySettings(parent, { id, input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to update app security settings"
          );
        }
    ...
    async updateAppBrandingSettings(parent, { id, input }, context) {
      ...
      try {
        ...
        if (!hasOrgAccess && !hasAppAccess && !isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Insufficient permissions to update app branding settings"
          );
        }
  ...
  Subscription: {
    appUpdated: {
      subscribe: withFilter(
        (parent, args, context) =>
          context.pubsub.asyncIterator(["APP_UPDATED"]),
    ...
    appMembershipChanged: {
      subscribe: withFilter(
        (parent, args, context) =>
          context.pubsub.asyncIterator(["APP_MEMBERSHIP_CHANGED"]),
        async (payload, variables, context) => {
          if (!context.user) return false;
  ...
  App: {
    ...
    organizationId(parent) {
      ...
      if (parent.organizationId && typeof parent.organizationId === "object") {
        if (parent.organizationId._id) {
          return parent.organizationId._id.toString();
        }
        if (parent.organizationId.toString) {
          return parent.organizationId.toString();
        }
      }
      return parent.organizationId ? parent.organizationId.toString() : null;
    },
    ...
    async organization(parent, args, context) {
      ...
      try {
        if (dataLoader?.organizationLoader) {
          return await dataLoader.organizationLoader.load(
    ...
    async owner(parent, args, context) {
      ...
      try {
        ...
        if (!ownerId) {
          console.error("App has no owner ID:", {
          ...
          return {
            ...
            createdAt: new Date(0).toISOString(), // Placeholder epoch
    ...
    authSettings(parent) {
      ...
      return {
        ...
        requireEmailVerification:
          settings.auth?.requireEmailVerification ?? true,
