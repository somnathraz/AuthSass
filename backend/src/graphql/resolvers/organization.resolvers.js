...
const organizationResolvers = {
  Query: {
    ...
    async organizations(
      parent,
      { limit, offset, sortBy, sortOrder, filter },
      context
    ) {
      const { user } = context;
  ...
  Mutation: {
    async createOrganization(parent, { input }, context) {
      ...
      try {
        ...
        // Audit log: Customer tier for organization creation
    ...
    async updateOrganization(parent, { id, input }, context) {
      ...
      try {
        ...
        // Audit log: Customer tier for organization update
        await auditCustomerLog("ORGANIZATION_UPDATED", id, user.id, {
          category: "ORGANIZATION",
          actorType: "USER",
    ...
    async deleteOrganization(parent, { id }, context) {
      ...
      try {
        ...
        if (!isOwner && !isSystemAdmin) {
          throw new ForbiddenError(
            "Only organization owner or system admin can delete organization"
          );
        }

        await OrganizationService.deleteOrganization(id, user.id);

        // Audit log: Customer tier for organization deletion
        await auditCustomerLog("ORGANIZATION_DELETED", id, user.id, {
          category: "ORGANIZATION",
          actorType: "USER",
    ...
    async switchOrganization(parent, { orgId }, context) {
      ...
      try {
        ...
        // Check if user has access to this organization
        const hasAccess = await PermissionService.checkUserOrgAccess(
          user.id,
          orgId
        );
    ...
    async updatePasswordPolicy(parent, { id, input }, context) {
      ...
      try {
        ...
        if (
          ...
          throw new ForbiddenError(
            "Insufficient permissions to update password policy"
          );
        }
        ...
        const organization = await OrganizationService.updatePasswordPolicy(
          id,
          input,
          user.id
        );
    ...
    async updateDomainSettings(parent, { id, input }, context) {
      ...
      try {
        ...
        if (
          ...
          throw new ForbiddenError(
            "Insufficient permissions to update domain settings"
          );
        }
        ...
        const organization = await OrganizationService.updateDomainSettings(
          id,
          input,
          user.id
        );
    ...
    async updateBrandingSettings(parent, { id, input }, context) {
      ...
      try {
        ...
        const organization = await OrganizationService.updateBrandingSettings(
          id,
          input,
          user.id
        );
    ...
    async updateNotificationSettings(parent, { id, input }, context) {
      ...
      try {
        ...
        const organization =
          await OrganizationService.updateNotificationSettings(
            id,
            input,
            user.id
          );
    ...
    async updateAnalyticsSettings(parent, { id, input }, context) {
      ...
      try {
        ...
        if (
          ...
          throw new ForbiddenError(
            "Insufficient permissions to update analytics settings"
          );
        }
        ...
        const organization = await OrganizationService.updateAnalyticsSettings(
          id,
          input,
          user.id
        );
  ...
  Subscription: {
    organizationUpdated: {
      subscribe: withFilter(
        (parent, args, context) =>
          context.pubsub.asyncIterator(["ORGANIZATION_UPDATED"]),
    ...
    organizationMembershipChanged: {
      subscribe: withFilter(
        (parent, args, context) =>
          context.pubsub.asyncIterator(["ORGANIZATION_MEMBERSHIP_CHANGED"]),
        async (payload, variables, context) => {
          if (!context.user) return false;
